...
public class GameController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for(int i=0; i< columns; i++)
        {
            for(int j=0; j<rows; j++)
            {
                ...
                float postitionX = (Xspace * i) + startPosition.x;
                float postitionY = (Yspace * j) + startPosition.y;

                gameImage.transform.position = new Vector3(postitionX,postitionY, startPosition.z);
            }
        }
    }
    ...
    private IEnumerator CheckGuessed()
    {
        ...
        if (attempts > 12) // Verifica si se alcanzó el límite de intentos.
        {
